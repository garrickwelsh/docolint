...
Languages with doc comment conventions (JS/TS/Java): extract `/** */` by default. `//` inline comments extracted only when `include_inline_comments` config is `true`. Languages without doc comment conventions (Bash, PowerShell, SCSS, CSS, Lua, Python): extract all comments regardless of config.

### 2. Configurable inline comment inclusion
New `include_inline_comments` option added to `InitializationOptions` (default: `false`). Flows through `ServerState` → `ParserConfig` → `extract_comment_docs`. Only affects languages with doc comment distinctions.
